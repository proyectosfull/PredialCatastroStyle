...
{
    public partial class BusquedaDiferencia : System.Web.UI.Page
    {
        ...
        {                     
            ...
            {      
                ...
                if (parametros != null)
                {
                    ...
                    {
                        if (parametros["origen"] == "BusquedaDiferencia")
                        {
        ...
        {
            ddlFiltro.DataSource = new cDiferenciaBL().ListaCampos();
            ddlFiltro.DataBind();
        ...
        private void llenagrid()
        {
            ...
            if (filtro == null)
            {
                grd.DataSource = new cDiferenciaBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                cPredio p = new cPredioBL().GetByClavePredial(filtro[1]);
                if (!(p is null))
                {
                    grd.DataSource = new cDiferenciaBL().GetFilter(filtro[0], p.Id.ToString(), filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                    grd.DataBind();
                }
                else
                {
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "ActivarRegistro")
            {
                ...
                List<cDiferencia> diferencias = new cDiferenciaBL().GetListDiferenciaByPredio(diferencia.IdPredio);
                if (diferencias.Count() > 0)
                {
                    string mensaje = "Imposible activar la Diferncia, ya existe activa una diferencia con la misma clave catastral.";
        ...
        protected void grd_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                int idPredio = int.Parse(e.Row.Cells[0].Text);
                e.Row.Cells[0].Text = new cPredioBL().GetByConstraint(idPredio).ClavePredial;
        ...
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            {
                cDiferencia Diferencia = new cDiferenciaBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                Diferencia.Activo = false;
                Diferencia.IdUsuario = U.Id;
                Diferencia.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cDiferenciaBL().Delete(Diferencia);
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                Diferencia.IdUsuario = U.Id;
                Diferencia.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cDiferenciaBL().Update(Diferencia);
                vtnModal.DysplayCancelar = true;
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
        ...
        protected void imbBuscar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            {               
                string[] filtro = new string[] { ddlFiltro.SelectedValue, txtFiltro.Text.Trim(), chkInactivo.Checked.ToString() };
                ViewState["filtro"] = filtro;
                llenagrid();
            }
            else
            {
                vtnModal.ShowPopup(new Utileria().GetDescription(MensajesInterfaz.ClaveMinOcho), ModalPopupMensaje.TypeMesssage.Alert);
            }
        }
