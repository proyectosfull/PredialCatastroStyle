...
{
    public partial class CatAgentesFiscales1 : System.Web.UI.Page
    {
        ...
        {
            ddlFiltro.DataSource = new cAgenteFiscalBL().ListaCampos();
            ddlFiltro.DataBind();
        ...
        private void llenagrid()
        {
            ...
            if (filtro == null)
            {
                grd.DataSource = new cAgenteFiscalBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                grd.DataSource = new cAgenteFiscalBL().GetFilter(filtro[0], filtro[1], filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                lbl_titulo.Text = "Consulta Agente Fiscal";
                pnl_Modal.Show();
                int id = Convert.ToInt32(e.CommandArgument);
                cAgenteFiscal Agente = new cAgenteFiscalBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(false);
                txtNombre.Text = Agente.Nombre.ToString();
                txtPaterno.Text = Agente.ApellidoPaterno.ToString();
                txtMaterno.Text = Agente.ApellidoMaterno.ToString();
                txtDireccion.Text = Agente.Direccion.ToString();
                txtCiudad.Text = Agente.Ciudad.ToString();
                txtEstado.Text = Agente.Estado.ToString();
                //txtSexo.Text = Agente.Sexo.ToString();
                ddlSexo.SelectedValue = Agente.Sexo.ToString();
                txtTel.Text = Agente.Telefono.ToString();
                txtEmail.Text = Agente.Email.ToString();
                txtRfc.Text = Agente.Rfc.ToString();
                txtCurp.Text = Agente.Curp.ToString();
                txtFechaIngreso.Text = Agente.FechaIngreso.ToString();
                btnGuardar.Visible = false;
            }
            else if (e.CommandName == "ModificarRegistro")
            {
                lbl_titulo.Text = "Modifica Agente Fiscal";
                int id = Convert.ToInt32(e.CommandArgument);
                cAgenteFiscal Agente = new cAgenteFiscalBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(true);
        ...
        {
            lbl_titulo.Text = "Alta Agente Fiscal";
            pnl_Modal.Show();
            limpiaCampos();
            habilitaCampos(true);
            btnGuardar.Visible = true;
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
                ...
                {
                    cAgenteFiscal Agente = new cAgenteFiscal();
                    Agente.Nombre = txtNombre.Text;
                    Agente.ApellidoPaterno = txtPaterno.Text;
                    Agente.ApellidoMaterno = txtMaterno.Text;
                    Agente.Direccion = txtDireccion.Text;
                    Agente.Ciudad = txtCiudad.Text;
                    Agente.Estado = txtEstado.Text;
                    Agente.Sexo = ddlSexo.SelectedValue.ToString();
                    Agente.Telefono = txtTel.Text;
                    Agente.Email = txtEmail.Text;
                    Agente.Rfc = txtRfc.Text;
                    Agente.Curp = txtCurp.Text;
                    Agente.FechaIngreso = Convert.ToDateTime(txtFechaIngreso.Text);
                    Agente.IdUsuario = U.Id;
                    Agente.Activo = true;
                    Agente.FechaModificacion = DateTime.Now;
                    
                    MensajesInterfaz msg = new cAgenteFiscalBL().Insert(Agente);
                    vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
                }
                else
                {
                    cAgenteFiscal Agente = new cAgenteFiscalBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                    Agente.Nombre = txtNombre.Text;
                    Agente.ApellidoPaterno = txtPaterno.Text;
                    Agente.ApellidoMaterno = txtMaterno.Text;
                    Agente.Direccion = txtDireccion.Text;
                    ...
                    Agente.IdUsuario = U.Id;
                    Agente.FechaModificacion = DateTime.Now;
                    MensajesInterfaz msg = new cAgenteFiscalBL().Update(Agente);
                    vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
                }
            limpiaCampos();
            llenagrid();
        ...
        private void habilitaCampos(bool activa)
        {
            ...
            txtPaterno.Enabled = activa;
            txtMaterno.Enabled = activa;
            txtDireccion.Enabled = activa;
            txtCiudad.Enabled = activa;
            txtEstado.Enabled = activa;
            ddlSexo.Enabled = activa;
            txtTel.Enabled = activa;
            txtEmail.Enabled = activa;
            txtRfc.Enabled = activa;
            txtCurp.Enabled = activa;
            txtFechaIngreso.Enabled = activa;
        }
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ConfimacionEliminar))
            {
                ...
                Agente.IdUsuario = U.Id;
                Agente.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cAgenteFiscalBL().Delete(Agente);
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                Agente.IdUsuario = U.Id;
                Agente.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cAgenteFiscalBL().Update(Agente);
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
            }
