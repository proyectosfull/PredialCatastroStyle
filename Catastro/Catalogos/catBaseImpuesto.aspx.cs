...
{
    public partial class catBaseImpuesto : System.Web.UI.Page
    {
        ...
        {           
            ddlFiltro.DataSource = new cBaseImpuestoBL().ListaCampos();
            ddlFiltro.DataBind();
        ...
        private void llenagrid()
        {
          string[] filtro =  (string[])ViewState["filtro"];
            ...
            if (filtro == null)
            {
                grd.DataSource = new cBaseImpuestoBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                grd.DataSource = new cBaseImpuestoBL().GetFilter(filtro[0], filtro[1], filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                lbl_titulo.Text = "Consulta Base Impuesto";
                pnl_Modal.Show();
                int id = Convert.ToInt32(e.CommandArgument);
                cBaseImpuesto cBaseImpuesto = new cBaseImpuestoBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(false);
                txtEjercicio.Text = cBaseImpuesto.Ejercicio.ToString();
                txtImporte.Text = cBaseImpuesto.Valor.ToString();
                btnGuardar.Visible = false;
            }
          else if (e.CommandName == "ModificarRegistro")
            {
                lbl_titulo.Text = "Modifica Base Impuesto";
                int id = Convert.ToInt32(e.CommandArgument);
                cBaseImpuesto cBaseImpuesto = new cBaseImpuestoBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(true);
        ...
        {
            lbl_titulo.Text = "Alta de Nueva Base de Impuesto";
            pnl_Modal.Show();
            limpiaCampos();
            habilitaCampos(true);
            btnGuardar.Visible = true;
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                //double i = 2.6;
                cBaseImpuesto cBaseImpuesto = new cBaseImpuesto();
                cBaseImpuesto.Ejercicio = Convert.ToInt32(txtEjercicio.Text);
                cBaseImpuesto.Valor = toDecimal(txtImporte.Text);
                cBaseImpuesto.IdUsuario = U.Id;
                cBaseImpuesto.Activo = true;
                cBaseImpuesto.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cBaseImpuestoBL().Insert(cBaseImpuesto);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
            else
            {
                cBaseImpuesto cBaseImpuesto = new cBaseImpuestoBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                cBaseImpuesto.Ejercicio = Convert.ToInt32(txtEjercicio.Text);
                cBaseImpuesto.Valor = toDecimal(txtImporte.Text);
                cBaseImpuesto.IdUsuario = U.Id;
                cBaseImpuesto.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cBaseImpuestoBL().Update(cBaseImpuesto);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
        ...
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            {
                cBaseImpuesto cBaseImpuesto = new cBaseImpuestoBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                cBaseImpuesto.Activo = false;
                cBaseImpuesto.IdUsuario = U.Id;
                cBaseImpuesto.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cBaseImpuestoBL().Delete(cBaseImpuesto);               
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                cBaseImpuesto.IdUsuario = U.Id;
                cBaseImpuesto.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cBaseImpuestoBL().Update(cBaseImpuesto);                
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
            }
