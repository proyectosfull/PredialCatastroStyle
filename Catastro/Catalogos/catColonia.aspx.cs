...
namespace Transito.Catalogos
{
    public partial class catColonia : System.Web.UI.Page
    {
        ...
        {
            List<cTipoAsentamiento> ct = new cTipoAsentamientoBL().GetAll();
            ListItem m = new ListItem("SELECCIONA TIPO ASENTAMIENTO", "0");
            ddlAsentamientos.Items.Add(m);
            foreach (cTipoAsentamiento a in ct){
                m = new ListItem(a.descripcion,a.Id.ToString());
                ddlAsentamientos.Items.Add(m);
            }
        }
        ...
        {           
            ddlFiltro.DataSource = new cColoniaBL().ListaCampos();
            ddlFiltro.DataBind();
        ...
        private void llenagrid()
        {
          string[] filtro =  (string[])ViewState["filtro"];
            ...
            if (filtro == null)
            {
                grd.DataSource = new cColoniaBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                grd.DataSource = new cColoniaBL().GetFilter(filtro[0], filtro[1], filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                lbl_titulo.Text = "Consulta Colonia";
                pnl_Modal.Show();
                int id = Convert.ToInt32(e.CommandArgument);
                cColonia cColonia = new cColoniaBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(false);
                txtNombreColonia.Text = cColonia.NombreColonia;
                txtCP.Text = cColonia.Cp;
                try
                {
                    ddlAsentamientos.SelectedValue = cColonia.IdTipoAsentamiento.ToString();
                }
                catch 
                {
                    ddlAsentamientos.SelectedValue = "0";
                }
                btnGuardar.Visible = false;
            }
          else if (e.CommandName == "ModificarRegistro")
            {
                lbl_titulo.Text = "Modifica Colonia";
                int id = Convert.ToInt32(e.CommandArgument);
                cColonia cColonia = new cColoniaBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(true);
        ...
        {
            lbl_titulo.Text = "Alta de Nueva Colonia";
            pnl_Modal.Show();
            limpiaCampos();
            habilitaCampos(true);
            btnGuardar.Visible = true;
            txtNombreColonia.Focus();
        }
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (ViewState["idMod"] == null || ViewState["idMod"].ToString() == string.Empty || ViewState["idMod"].ToString() == "0")
            {
                ...
                cColonia.NombreColonia = txtNombreColonia.Text;
                cColonia.Cp = txtCP.Text;
                cColonia.IdTipoAsentamiento = Convert.ToInt32(ddlAsentamientos.SelectedValue);
                cColonia.IdUsuario = U.Id;
                cColonia.Activo = true;
                cColonia.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cColoniaBL().Insert(cColonia);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
            else
            {
                cColonia cColonia = new cColoniaBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                cColonia.NombreColonia = txtNombreColonia.Text;
                cColonia.Cp = txtCP.Text;
                cColonia.IdTipoAsentamiento = Convert.ToInt32(ddlAsentamientos.SelectedValue);
                cColonia.IdUsuario = U.Id;
                cColonia.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cColoniaBL().Update(cColonia);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
        ...
        {
            txtNombreColonia.Enabled = activa;
            txtCP.Enabled = activa;
            ddlAsentamientos.Enabled = activa;
        }
        ...
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            {
                cColonia cColonia = new cColoniaBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                cColonia.Activo = false;
                cColonia.IdUsuario = U.Id;
                cColonia.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cColoniaBL().Delete(cColonia);               
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                cColonia.IdUsuario = U.Id;
                cColonia.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cColoniaBL().Update(cColonia);                
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
            }
