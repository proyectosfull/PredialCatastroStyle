...
{
    public partial class catCondominio : System.Web.UI.Page
    {
        ...
        {           
            ddlFiltro.DataSource = new cCondominioBL().ListaCampos();
            ddlFiltro.DataBind();
        ...
        private void llenagrid()
        {
          string[] filtro =  (string[])ViewState["filtro"];
            ...
            if (filtro == null)
            {
                grd.DataSource = new cCondominioBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                grd.DataSource = new cCondominioBL().GetFilter(filtro[0], filtro[1], filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                lbl_titulo.Text = "Consulta Condominio";
                pnl_Modal.Show();
                int id = Convert.ToInt32(e.CommandArgument);
                cCondominio cCondominio = new cCondominioBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(false);
                txtNombreCondominio.Text = cCondominio.Descripcion;               
             
                btnGuardar.Visible = false;
            }
          else if (e.CommandName == "ModificarRegistro")
            {
                lbl_titulo.Text = "Modifica Condominio";
                int id = Convert.ToInt32(e.CommandArgument);
                cCondominio cCondominio = new cCondominioBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(true);
        ...
        {
            lbl_titulo.Text = "Alta de Nueva Condominio";
            pnl_Modal.Show();
            limpiaCampos();
            habilitaCampos(true);
            btnGuardar.Visible = true;
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                cCondominio cCondominio = new cCondominio();
                cCondominio.Descripcion = txtNombreCondominio.Text;               
                cCondominio.IdUsuario = U.Id;
                cCondominio.Activo = true;
                cCondominio.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cCondominioBL().Insert(cCondominio);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
            else
            {
                cCondominio cCondominio = new cCondominioBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                cCondominio.Descripcion = txtNombreCondominio.Text;
                cCondominio.IdUsuario = U.Id;
                cCondominio.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cCondominioBL().Update(cCondominio);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
        ...
        {
            txtNombreCondominio.Enabled = activa;           
        }
        ...
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            {
                cCondominio cCondominio = new cCondominioBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                cCondominio.Activo = false;
                cCondominio.IdUsuario = U.Id;
                cCondominio.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cCondominioBL().Delete(cCondominio);               
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                cCondominio.IdUsuario = U.Id;
                cCondominio.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cCondominioBL().Update(cCondominio);                
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
            }
