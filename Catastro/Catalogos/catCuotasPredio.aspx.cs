...
{
    public partial class catCuotasPredio : System.Web.UI.Page
    {
        ...
        {
            ddlFiltro.DataSource = new cCuotasPredioBL().ListaCampos();
            ddlFiltro.DataBind();
            ddlFiltro.Items.Insert(0, new ListItem("Todos", ""));
        }

        private void llenaTipoPredio()
        {
            ddlTipoPredio.Items.Clear();
            ...
            ddlTipoPredio.Items.Insert(0, new ListItem("Seleccionar Cargo", "%"));
        ...
        private void llenagrid()
        {
            ...
            if (filtro == null)
            {
                grd.DataSource = new cCuotasPredioBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                grd.DataSource = new cCuotasPredioBL().GetFilter(filtro[0], filtro[1], filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "ConsultarRegistro")
            {
                llenaTipoPredio();
                llenarTipoCobro();
                lbl_titulo.Text = "Consulta de la Cuota Predio";
                pnl_Modal.Show();
                int id = Convert.ToInt32(e.CommandArgument);
                cCuotasPredio CuotasPredio = new cCuotasPredioBL().GetByConstraint(id);
                limpiaCampos();                
                habilitaCampos(false);
                txtEjercicio.Text = CuotasPredio.Ejercicio.ToString();
                if (CuotasPredio.cTipoPredio.Activo)
                {
                    ddlTipoPredio.SelectedValue = CuotasPredio.cTipoPredio.Id.ToString();
                }
                else
                {
                    ddlTipoPredio.Items.Add(new ListItem(CuotasPredio.cTipoPredio.Descripcion, CuotasPredio.cTipoPredio.Id.ToString()));
                    ddlTipoPredio.SelectedValue = CuotasPredio.cTipoPredio.Id.ToString();

                }

                cTipoCobro tipoCobro = new cTipoCobroBL().GetByConstraint(CuotasPredio.IdTipoCobro);
                if (tipoCobro != null)
                    if (tipoCobro.Activo)
                    {
                        ddlTipoCobro.SelectedValue = CuotasPredio.cTipoCobro.Id.ToString();
                    }
                    else
                    {
                ...
                txtCoutaCobro.Text = CuotasPredio.CuotasCobro.ToString();                
            ...
            else if (e.CommandName == "ModificarRegistro")
            {
                llenaTipoPredio();
                llenarTipoCobro();
                lbl_titulo.Text = "Modificación de la Cuota Predio";
                int id = Convert.ToInt32(e.CommandArgument);
                cCuotasPredio CuotasPredio = new cCuotasPredioBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(true);
        ...
        protected void grd_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                int idTipoPredio = int.Parse(e.Row.Cells[3].Text);
                e.Row.Cells[3].Text = new cTipoPredioBL().GetByConstraint(idTipoPredio).Descripcion;                                
        ...
        protected void btnNuevo_Click(object sender, EventArgs e)
        {
            llenaTipoPredio();
            llenarTipoCobro();
            lbl_titulo.Text = "Alta de la Couta Predio";
            pnl_Modal.Show();
            limpiaCampos();
            habilitaCampos(true);
            btnGuardar.Visible = true;
        ...
        {            
            ...
            {
                cCuotasPredio CuotasPredio = new cCuotasPredio();
                CuotasPredio.Ejercicio =Convert.ToInt32(txtEjercicio.Text);
                CuotasPredio.IdTipoPredio = Convert.ToInt32(ddlTipoPredio.SelectedValue);
                CuotasPredio.CuotasCobro =  Convert.ToDecimal(txtCoutaCobro.Text);
                CuotasPredio.IdTipoCobro = Convert.ToInt32(ddlTipoCobro.SelectedValue);
                CuotasPredio.IdUsuario = U.Id;
                CuotasPredio.Activo = true;
                CuotasPredio.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cCuotasPredioBL().Insert(CuotasPredio);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
            else
            {
                cCuotasPredio CuotasPredio = new cCuotasPredioBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                CuotasPredio.Ejercicio = Convert.ToInt32(txtEjercicio.Text);
                CuotasPredio.IdTipoPredio = Convert.ToInt32(ddlTipoPredio.SelectedValue);
                CuotasPredio.CuotasCobro = Convert.ToDecimal(txtCoutaCobro.Text);
                CuotasPredio.IdTipoCobro =  Convert.ToInt32(ddlTipoCobro.SelectedValue);
                CuotasPredio.IdUsuario = U.Id;
                CuotasPredio.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cCuotasPredioBL().Update(CuotasPredio);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
            limpiaCampos();
            llenagrid();
        ...
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            {
                cCuotasPredio CuotasPredio = new cCuotasPredioBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                CuotasPredio.Activo = false;
                CuotasPredio.IdUsuario = U.Id;
                CuotasPredio.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cCuotasPredioBL().Delete(CuotasPredio);
                vtnModal.DysplayCancelar = true;
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                CuotasPredio.IdUsuario = U.Id;
                CuotasPredio.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cCuotasPredioBL().Update(CuotasPredio);
                vtnModal.DysplayCancelar = true;
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
