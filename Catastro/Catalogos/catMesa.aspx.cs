...
{
    public partial class catMesa : System.Web.UI.Page
    {
        ...
        {
            ddlFiltro.DataSource = new cMesaBL().ListaCampos();
            ddlFiltro.DataBind();
        ...
        private void llenagrid()
        {
            ...
            if (filtro == null)
            {
                grd.DataSource = new cMesaBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                grd.DataSource = new cMesaBL().GetFilter(filtro[0], filtro[1], filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                lbl_titulo.Text = "Consulta Mesa";
                pnl_Modal.Show();
                int id = Convert.ToInt32(e.CommandArgument);
                cMesa mesa = new cMesaBL().GetByConstraint(id);
                limpiaCampos();
        ...
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ConfimacionEliminar))
            {
                ...
                mesa.IdUsuario = U.Id;
                mesa.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cMesaBL().Delete(mesa);
                vtnModal.DysplayCancelar = false;
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                mesa.IdUsuario = U.Id;
                mesa.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cMesaBL().Update(mesa);
                vtnModal.DysplayCancelar = false;
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
