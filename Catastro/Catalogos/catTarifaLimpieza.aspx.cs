...
{
    public partial class CatTarifaLimpieza : System.Web.UI.Page
    {
        ...
        {
            ddlFiltro.DataSource = new cTarifaLimpiezaBL().ListaCampos();
            ddlFiltro.DataBind();
        ...
        private void llenagrid()
        {
            ...
            if (filtro == null)
            {
                grd.DataSource = new cTarifaLimpiezaBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                grd.DataSource = new cTarifaLimpiezaBL().GetFilter(filtro[0], filtro[1], filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                lbl_titulo.Text = "Consulta Tarifa de Limpieza";
                pnl_Modal.Show();
                int id = Convert.ToInt32(e.CommandArgument);
                cTarifaLimpieza Tarifa = new cTarifaLimpiezaBL().GetByConstraint(id);
                limpiaCampos();
            ...
            {
                lbl_titulo.Text = "Modifica Tarifa de Limpieza";
                int id = Convert.ToInt32(e.CommandArgument);
                cTarifaLimpieza Tarifa = new cTarifaLimpiezaBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(true);
        ...
        {
            lbl_titulo.Text = "Alta Tarifa de Limpieza";
            pnl_Modal.Show();
            limpiaCampos();
            habilitaCampos(true);
            btnGuardar.Visible = true;
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                cTarifaLimpieza Tarifa = new cTarifaLimpieza();
                ...
                MensajesInterfaz msg = new cTarifaLimpiezaBL().Insert(Tarifa);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
            else
            {
                cTarifaLimpieza Tarifa = new cTarifaLimpiezaBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                ...
                Tarifa.Activo = true;
                Tarifa.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cTarifaLimpiezaBL().Update(Tarifa);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
            limpiaCampos();
            llenagrid();
        ...
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ConfimacionEliminar))
            {
                ...
                Tarifa.IdUsuario = U.Id;
                Tarifa.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cTarifaLimpiezaBL().Delete(Tarifa);
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                Tarifa.IdUsuario = U.Id;
                Tarifa.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cTarifaLimpiezaBL().Update(Tarifa);
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
            }
