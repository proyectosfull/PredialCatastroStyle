...
{
    public partial class catValuador : System.Web.UI.Page
    {
        ...
        {
            ddlFiltro.DataSource = new cValuadorBL().ListaCampos();
            ddlFiltro.DataBind();
        ...
        private void llenagrid()
        {
            ...
            if (filtro == null)
            {
                grd.DataSource = new cValuadorBL().GetFilter("", "", "TRUE", ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
            }
            else
            {
                grd.DataSource = new cValuadorBL().GetFilter(filtro[0], filtro[1], filtro[2], ViewState["sortCampo"].ToString(), ViewState["sortOnden"].ToString());
                grd.DataBind();
        ...
        protected void grd_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                lbl_titulo.Text = "Consulta Valuador";
                pnl_Modal.Show();
                int id = Convert.ToInt32(e.CommandArgument);
                cValuador cValuador = new cValuadorBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(false);
                txtNombre.Text = cValuador.nombre;
                txtApellidoPaterno.Text = cValuador.ApellidoPaterno;
                txtApellidoMaterno.Text = cValuador.ApellidoMaterno;
                btnGuardar.Visible = false;
            }
            else if (e.CommandName == "ModificarRegistro")
            {
                lbl_titulo.Text = "Modifica Valuador";
                int id = Convert.ToInt32(e.CommandArgument);
                cValuador cValuador = new cValuadorBL().GetByConstraint(id);
                limpiaCampos();
                habilitaCampos(true);
        ...
        {
            lbl_titulo.Text = "Alta de Nueva Valuador";
            pnl_Modal.Show();
            limpiaCampos();
            habilitaCampos(true);
            btnGuardar.Visible = true;
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                cValuador Valuador = new cValuador();
                Valuador.nombre = txtNombre.Text;
                Valuador.ApellidoPaterno = txtApellidoPaterno.Text;
                Valuador.ApellidoMaterno = txtApellidoMaterno.Text;
                Valuador.IdUsuario = U.Id;
                Valuador.Activo = true;
                Valuador.FechaModificacion = DateTime.Now;                
                MensajesInterfaz msg = new cValuadorBL().Insert(Valuador);                
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
            else
            {
                cValuador Valuador = new cValuadorBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                Valuador.nombre = txtNombre.Text;
                Valuador.ApellidoPaterno = txtApellidoPaterno.Text;
                Valuador.ApellidoMaterno = txtApellidoMaterno.Text;
                Valuador.IdUsuario = U.Id;
                Valuador.FechaModificacion = DateTime.Now;
                MensajesInterfaz msg = new cValuadorBL().Update(Valuador);
                vtnModal.ShowPopup(new Utileria().GetDescription(msg), ModalPopupMensaje.TypeMesssage.Alert);
            }
        ...
        protected void vtnModal_eventoAceptar(object sender, EventArgs e)
        {
            ...
            {
                cValuador Valuador = new cValuadorBL().GetByConstraint(Convert.ToInt32(ViewState["idMod"]));
                Valuador.Activo = false;
                Valuador.IdUsuario = U.Id;
                Valuador.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cValuadorBL().Delete(Valuador);
                vtnModal.DysplayCancelar = true;
            ...
            else if (vtnModal.Mensaje == new Utileria().GetDescription(MensajesInterfaz.ActivarRegistro))
            {
                ...
                Valuador.IdUsuario = U.Id;
                Valuador.FechaModificacion = DateTime.Now;
                MensajesInterfaz resul = new cValuadorBL().Update(Valuador);
                vtnModal.DysplayCancelar = true;
                vtnModal.ShowPopup(new Utileria().GetDescription(resul), ModalPopupMensaje.TypeMesssage.Alert);
                ViewState["idMod"] = 0;
                llenagrid();
