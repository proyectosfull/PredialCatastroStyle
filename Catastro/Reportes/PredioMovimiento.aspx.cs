...
{
	public partial class PredioMovimiento : System.Web.UI.Page
	{
        ...
        //    }
        //    //foreach (int? anio in new vVistasBL().ObtienePadronAnios(int.Parse(ddlStatus.SelectedValue), int.Parse(ddlBimestre.SelectedValue), int.Parse(ddlTipo.SelectedValue)))
        //    //{
        ...
        protected void imbBuscar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            List<vPredioMovimiento> listado = new List<vPredioMovimiento>();
            DateTime fechaFin = Convert.ToDateTime(txtFechaFin.Text + " 23:59:59");
            DateTime fechaIni = Convert.ToDateTime(txtFechaInicio.Text);

            if (txtInicioClave.Text != "" && txtFinClave.Text != "")
            {
                if (txtInicioClave.Text.Count() != 12)
                {
                    lblInicio.Visible = true;
                }
                if (txtFinClave.Text.Count() != 12)
                {
                    lblFinal.Visible = true;
                }
                String[] substringsinicio = txtInicioClave.Text.Split();
                String[] substringsfinal = txtFinClave.Text.Split();
                int i = 0;
                string clv = "";
                foreach (var s in substringsinicio)
                {
                    if (s == substringsfinal[i])
                    {
                        clv = clv + s;
                    }
                    i = i + 1;
                ...
                listado = new vVistasBL().vPredioMovimiento(fechaIni, fechaFin, 0, hdfIdCondominio.Value, txtColonia.Text, txtInicioClave.Text, txtFinClave.Text);
                    //(int.Parse(ddlStatus.SelectedValue), int.Parse(ddlAnio.SelectedValue), int.Parse(ddlBimestre.SelectedValue), int.Parse(ddlTipo.SelectedValue), clv, RemoveSpecialCharacters(txtClave.Text), txtContribuyente.Text.Trim(), hdfIdCondominio.Value, txtColonia.Text, txtInicioClave.Text, txtFinClave.Text);
            }
            else if (txtInicioClave.Text == "" && txtFinClave.Text != "")
            {
                lblInicio.Visible = true;
            }
            else if (txtInicioClave.Text != "" && txtFinClave.Text == "")
            ...
            rpvtPredios.LocalReport.DisplayName = "Movimientos Catastrales";
            rpvtPredios.LocalReport.ReportPath = "Reportes/predioMovimiento.rdlc";
            ...
            rpvtPredios.LocalReport.DataSources.Add(new ReportDataSource("predioMov", listado));
